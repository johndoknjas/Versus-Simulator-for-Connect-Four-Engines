...
namespace EngineVersion17
{

// WHEN COPYING A CLASS IN, MAKE SURE IT INHERITANCES FROM CLASS TOOL!!!
...
void position::smart_evaluation()
{
    ...
    for (const coordinate_and_value& current: info_for_comp_valid_amplifying_squares)
    {
        ...
        {
            temp_evaluation_as_double += (static_cast<double>(current.value) * 0.5);
        }
                                                ...
                                                vector<vector<char>>& copy_board, vector<coordinate_and_value>& recorder) const
{
    ...
    const int small_amount = 3; // how many points to add if an amplifying square completes a 2-in-a-row.
    ...
    for (const treasure_spot& space: squares_amplifying_2) // running through spaces completing a 2-in-a-row.
    {
            ...
            next_square.col >= 0 && next_square.col <= max_col_index)
        {
            ...
            if (copy_board[next_square.row][next_square.col] == piece) // next_square is the same piece as those in the 2-in-a-row...
            {
                coordinate_and_value temp;
            ...
            else if (copy_board[next_square.row][next_square.col] == ' ' &&
            ...
            // else: copy_board[next_square.row][next_square.col] is enemy piece or just inaccessible,
