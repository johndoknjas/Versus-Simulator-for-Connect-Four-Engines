...
//    void set_future_positions(const vector<unique_ptr<position>>& future_positionsP);
    ...
    void fill_no_go(vector<vector<vector<char>>>& no_go); // Puts all future_positions that lose on the spot for comp in
                                                          // the no_go vector passed by reference.
    ...
    bool is_in_bounds(const coordinate& square); // returns true if square is in-bounds in board.
...
private:
    ...
    int find_individual_player_evaluation(const vector<treasure_spot>& squares_amplifying_3,
                                          const vector<treasure_spot>& squares_amplifying_2,
                                          char piece) const;
                                           // evaluates one of the player's position based off the squares completing their
                                           // 3-in-a-rows and 2-in-a-rows, and returns a +value representing their score.
                                           // piece is the char in the x-in-a-rows being examined (i.e., 'C' or 'U').
...
coordinate position::find_best_move_for_comp()
{
    ...
    vector<vector<vector<char>>> no_go; // stores boards that lose on the spot for the comp, since the user can win in one move.

    fill_no_go(no_go); // function will fill the no_go vector with the future_positions' boards that lose on the spot for comp.

    // Now let's test some conditions about no_go's size, to see if all moves lose, and ON THE SPOT to boot.

    if (no_go.size() == future_positions.size() && no_go.size() > 0)
    {
        // All moves/future_positions lose on the spot for the computer,
        // since the number of catastrophic positions = the number of future positions (so all future positions = BAD).

        // Also, the number of moves that lose on the spot is greater than zero.
        // If there are zero moves that lose on the spot, the computer is obviously not in trouble.

        // Since all moves lose on the spot, they're all equally bad. So, pick one randomly:

        int index = rand() % future_positions.size();

        return (future_positions[index]->last_move);
    }

    // All moves don't lose on the spot, so they aren't all equal.
    // Randomly pick a move with the same evaluation as the calling position object, and the move
    // must not lose on the spot (due to the above if statement, I know at least one such move exists).

    // Note: It's likely the choice will not be random, since probably only one move has the evaluation of the
    // calling object, due to smart_evaluation()'s detailed analysis. Still, might as well go through the positions
    // in a random order in case 2 or more positions happen to have the same evaluation.
   ...
   /* auto rng = default_random_engine {};
    ...
    {
        if (!is_element_in_vector(no_go, future_positions[index]->board) && future_positions[index]->evaluation == evaluation)
        {
            return (future_positions[index]->last_move);
        }
    }
...
void position::fill_no_go(vector<vector<vector<char>>>& no_go)
{
    for (int i = 0; i < future_positions.size(); i++)
    {
        // future_positions[i] represents the current move for the comp that I'm examining:

        // Now, I want to try all possible_moves of this new position, and see if there's any win for the opponent immediately.
        // If so, add this position's board to the no_go vector, since the comp loses on the spot if it goes into it.

        // First though, I must check that this future_positions[i] is not actually just a winning position for the comp
        // right now! If that's the case, it doesn't matter if the user can win in one move, since the comp has already won.

        if (future_positions[i]->did_computer_win())
        {
            continue; // jumps to the next iteration of the for loop, skipping the rest of the stuff below for this iteration.
                      // This ensures future_position[i]'s board will NOT be added to the no_go vector.
        }

        // I'm not running through the future_positions vector of this new position, since alpha-beta pruning would
        // have just kept one win for the opponent (which doesn't have to win on the spot).
        // This would have possibly kept a "long win", and pruned a win in one move.

        // Instead, I'll manually make each possible move in future_positions[i], adjusting future_position[i]'s attributes myself.

        // Before wrecking the attributes of future_positions[i], I should keep copies of them so that I can repair
        // future_positions[i] after the following for loop.

        vector<vector<char>> old_board = future_positions[i]->board;
        coordinate old_last_move = future_positions[i]->last_move;
        bool old_is_comp_turn = future_positions[i]->is_comp_turn;

        for (coordinate user_move: future_positions[i]->possible_moves)
        {
            future_positions[i]->board[user_move.row][user_move.col] = 'U'; // making the move
            future_positions[i]->last_move = user_move; // last_move now stores the move just made.
            future_positions[i]->is_comp_turn = true; // now it's the comp's turn since the "user" just moved two lines above.

            // TEST IF USER HAS WON HERE. If so, reset future_positions[i]'s attributes,
            // add future_position[i]'s board to no_go vector, and break.

            if (future_positions[i]->did_opponent_win())
            {
                future_positions[i]->board = old_board;
                future_positions[i]->last_move = old_last_move;
                future_positions[i]->is_comp_turn = old_is_comp_turn;

                no_go.push_back(future_positions[i]->board);

                break;
            }

            // Reset the changed data of future_positions[i] to what it was before:
                                                        ...
                                                        bool are_3_pieces, char piece)
{
 ...
 bool position::is_in_bounds(const coordinate& square)
 {
                                    ...
                                    bool starting_new_game)
{
    ...
    double thinking_time = 0.30; // Only 0.30 seconds, but the last iteration of the loop will make thinking time go higher.
...
void position::analyze_horizontal_perspective_of_last_move()
{
    ...
    if (num_pieces_in_a_row == 1) // "1-in-a-row"... adds no value, and not worth recording:
    {
        return;
    }
...
void position::smart_evaluation()
{
    ...
    int comp_score = find_individual_player_evaluation(squares_amplifying_comp_3, squares_amplifying_comp_2, 'C');
    int user_score = find_individual_player_evaluation(squares_amplifying_user_3, squares_amplifying_user_2, 'U');

    evaluation = comp_score - user_score;
}

int position::find_individual_player_evaluation(const vector<treasure_spot>& squares_amplifying_3,
                                                const vector<treasure_spot>& squares_amplifying_2,
                                                char piece) const // piece is the char in the x-in-a-rows being looked at.
{
    // Use row_barriers private member vector when seeing if an amplifying square should be counted.

    const int big_amount = 10; // how many points to add if there is an empty square that would complete a 3-in-a-row.
    const int small_amount = 3; // how many points to add if there is an empty square that would complete a 2-in-a-row.
    const char arbitrary_placeholder = 'A'; // inserted in any empty squares that can complete a 3-in-a-row.
    ...
    int score = 0; // individual score for the player (this value will be returned).

    vector<vector<char>> temp_board = board; // since I'll need to modify the board by filling in squares that complete
                                             // any 3-in-a-rows into 4-in-a-rows.
    ...
    for (const treasure_spot& space: squares_amplifying_3) // running through squares completing a 3-in-a-row.
    {
        ...
        if (temp_board[current_square.row][current_square.col] == ' ' &&
        ...
        {
            int value_of_square = big_amount * (current_square.row + 1 + num_pieces_per_column[current_square.col]);
            ...
            if ((current_square.row + 1 <= max_row_index && temp_board[current_square.row + 1][current_square.col] == 'A') ||
                (current_square.row - 1 >= 0 && temp_board[current_square.row - 1][current_square.col] == 'A'))
            {
                value_of_square *= stacked_threat_coefficient; // since the threat is much more powerful now.
            }

            score += value_of_square;

            temp_board[current_square.row][current_square.col] = arbitrary_placeholder;
        }
    }
    ...
    for (const treasure_spot& space: squares_amplifying_2) // running through spaces completing a 2-in-a-row.
    {
        ...
        if (temp_board[current_square.row][current_square.col] == ' ' &&
            (row_barriers[current_square.col] == UNDEFINED || row_barriers[current_square.col] <= current_square.row) &&
            next_square.row >= 0 && next_square.row <= max_row_index &&
            next_square.col >= 0 && next_square.col <= max_col_index)
        {
            // So the square amplifies a 2-in-a-row and is empty. Also, the next square is in-bounds, meaning it
            // may be possible to eventually create a 4-in-a-row. Let's examine the next square:

            // If next square is the same piece as those in the 2-in-a-row, then current_square
            // would make a 4-in-a-row if filled in. Add big value to this, and use stacked_threat_coefficient if applicable!

            // Else if next square is empty (and is accessible via finished column!), just add small value to score.

            // Else, next square is filled with the enemy piece, making a 4-in-a-row impossible. Add nothing to the score,
            // so don't even have an else statement (does nothing!).

            // This If-ElseIf situation is dealt with below:

            if (temp_board[next_square.row][next_square.col] == piece) // next_square is the same piece as those in the 2-in-a-row...
            {
            ...
            else if (temp_board[next_square.row][next_square.col] == ' ' &&
                     (row_barriers[next_square.col] == UNDEFINED || row_barriers[next_square.col] <= next_square.row))
            {
                ...
                // The next square is both empty and accessible (via finished column algorithm), so the
                // 2-in-a-row is at least worth something since it has the POTENTIAL to turn into a 4-in-a-row:

                score += small_amount * (current_square.row + 1 + num_pieces_per_column[current_square.col]);
                // since higher row index ==> lower on the board ==> better threat.
                // don't fill in square with 'A', since this square can only complete a 2-in-a-row into a 3-in-a-row.
                // It cannot join a 2-in-a-row with another piece to make a 4-in-a-row, since next_square is empty.
            }

            // else: temp_board[next_square.row][next_square.col] is enemy piece or just inaccessible,
